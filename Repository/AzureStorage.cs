...
using SumairaSanaullah_WebAPI.Services;
using SumairaSanaullah_WebAPI.Models;
using Azure;
using Azure.Identity;

namespace SumairaSanaullah_WebAPI.Repository
{
    public class AzureStorage : IAzureStorage
    {
        ...
        private readonly string _visualStudioTenantId;
        ...
        public AzureStorage(IConfiguration configuration, ILogger<AzureStorage> logger)
        {
            ...
            _visualStudioTenantId = configuration.GetValue<string>("VisualStudioTenantId");
            _logger = logger;
        }
        ...
        public async Task<List<Blob>> ListAsync()
        {
            ...
            var blobServiceClient = new BlobServiceClient(
                     new Uri(_storageConnectionString), 
                     new DefaultAzureCredential(
                         new DefaultAzureCredentialOptions()
                         {
                             VisualStudioTenantId = _visualStudioTenantId
                         }));
            ...
            {
                // Add each file retrieved from the storage container to the files list by creating a Blob object
        ...
        public async Task<Blob> DownloadAsync(string blobFilename)
        {
            ...
            try
            {
                ...
                if (await file.ExistsAsync())
                {
                    ...
                    // Add data to variables in order to return a Blob
                    string name = blobFilename;
                    string contentType = content.Value.Details.ContentType;

                    // Create new Blob with blob data from variables
